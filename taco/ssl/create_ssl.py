...
def get_covid_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "covid_ca.crt")
    key = pkg_resources.resource_string(__name__, "covid_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("covidnetwork.net")]),
            critical=False,
        )
...
def main():
    return make_ca_cert(Path("./covid_ca.crt"), Path("./covid_ca.key"))
