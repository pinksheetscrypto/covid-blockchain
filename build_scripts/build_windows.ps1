...
Write-Output "   ---"
# download.covidnetwork.org is the CDN url behind all the files that are actually on pypi.covidnetwork.org/simple now
...
Write-Output "   ---"
Copy-Item "dist\daemon" -Destination "..\covid-blockchain-gui\packages\gui\" -Recurse
...
Write-Output "   ---"
Copy-Item ".\Covid-win32-x64" -Destination "$env:GITHUB_WORKSPACE\covid-blockchain-gui\" -Recurse
Copy-Item ".\release-builds" -Destination "$env:GITHUB_WORKSPACE\covid-blockchain-gui\" -Recurse
