...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	DIR_NAME="covid-blockchain-linux-x64"
else
	#PLATFORM="$1"
	DIR_NAME="covid-blockchain-linux-arm64"
fi
...
--appVersion=$COVID_INSTALLER_VERSION --executable-name=covid-blockchain
LAST_EXIT_CODE=$?
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create covid-blockchain-$COVID_INSTALLER_VERSION.rpm"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/pinksheetscrypto/covid-blockchain/issues/3846
  ...
  --arch "$REDHAT_PLATFORM" --options.version $COVID_INSTALLER_VERSION \
  --license ../LICENSE --options.bin covid-blockchain --options.name covid-blockchain
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
