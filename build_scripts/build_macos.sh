...
if [ "$NOTARIZE" == true ]; then
  ...
  --hardened-runtime=true --provisioning-profile=covidblockchain.provisionprofile \
...
NODE_PATH=./npm_macos/node_modules node build_dmg.js dist/Covid-darwin-x64/Covid.app $COVID_INSTALLER_VERSION
LAST_EXIT_CODE=$?
if [ "$LAST_EXIT_CODE" -ne 0 ]; then
...
if [ "$NOTARIZE" == true ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id net.covid.blockchain \
...
# xcrun altool --notarize-app -f Covid-0.1.X.dmg --primary-bundle-id net.covid.blockchain -u username -p password
