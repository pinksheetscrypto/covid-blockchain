...
rm covid-blockchain-gui/temp.json || true
( cd "$PWD/covid-blockchain-gui" && git checkout HEAD -- package-lock.json ) || true
cd "$PWD" || true
...
npm uninstall -g @covid-network/notarize-cli || true
