...
from covid.clvm.spend_sim import SpendSim, SimClient
...
from covid.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
    construct_cat_puzzle,
    SpendableCAT,
    unsigned_spend_bundle_for_spendable_cats,
)
from covid.wallet.payment import Payment
from covid.wallet.trading.offer import Offer, NotarizedPayment
...
class TestOfferLifecycle:
    ...
    # This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/COV coins
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            covid_coins: List[Coin] = all_coins[None]
            red_coins: List[Coin] = all_coins["red"]
            blue_coins: List[Coin] = all_coins["blue"]

            # Create an COV Offer for RED
            covid_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            covid_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                covid_requested_payments, covid_coins
            )
            covid_announcements: List[Announcement] = Offer.calculate_announcements(covid_requested_payments)
            covid_secured_bundle: SpendBundle = self.generate_secure_bundle(covid_coins, covid_announcements, 1000)
            covid_offer = Offer(covid_requested_payments, covid_secured_bundle)
            assert not covid_offer.is_valid()

            # Create a RED Offer for COV
            ...
            new_offer = Offer.aggregate([covid_offer, red_offer])
            ...
            # Create yet another offer of BLUE for COV and RED
            ...
            assert new_offer.summary() == (
                ...
                {"cov": 900, str_to_tail_hash("red").hex(): 350},
            )
