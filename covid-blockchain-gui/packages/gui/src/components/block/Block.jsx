...
import { useGetBlockQuery, useGetBlockRecordQuery  } from '@covid/api-react'
...
export default function Block() {
  ...
  const poolReward = mojoToCovid(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToCovid(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const covidFees = blockRecord.fees !== undefined
    ? mojoToCovid(blockRecord.fees)
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.covidexplorer.org/blockchain/puzzlehash/${blockRecord.farmerPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.covidexplorer.org/blockchain/puzzlehash/${blockRecord.poolPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
