...
const getCovidVersion = () => {
  let version = null;
  const exePath = getExecutablePath('covid');
  // first see if we can get a covid exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or covid is on the path
    try {
      version = child_process
