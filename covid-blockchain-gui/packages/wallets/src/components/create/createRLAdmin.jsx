...
import { AlertDialog, covidToMojo } from '@covid/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      covidper_input.value === '' ||
      Number(covidper_input.value) === 0 ||
      !Number(covidper_input.value) ||
      isNaN(Number(covidper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const covidper = covidToMojo(covidper_input.value);
    const covidper_value = Number.parseInt(Number(covidper));
    const userpubkey = userpubkey_input.value;
    const amount = covidToMojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = covidToMojo(fee_input.value);
