...
import { type OfferSummaryRecord  } from '@covid/api';
import {
  Flex,
  StateColor,
  mojoToCovid,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries[0][0].toLowerCase() === 'cov' ? Number(mojoToCovid(makerEntries[0][1])) : Number(mojoToCAT(makerEntries[0][1]));
  const takerAmount = takerEntries[0][0].toLowerCase() === 'cov' ? Number(mojoToCovid(takerEntries[0][1])) : Number(mojoToCAT(takerEntries[0][1]));
