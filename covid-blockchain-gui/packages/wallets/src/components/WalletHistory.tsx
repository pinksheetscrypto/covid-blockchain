...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToCovidLocaleString, mojoToCATLocaleString } from '@covid/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@covid/api-react';
import styled from 'styled-components';
import type { Row } from '@covid/core';
import { WalletType, TransactionType } from '@covid/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToCovidLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToCovidLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
