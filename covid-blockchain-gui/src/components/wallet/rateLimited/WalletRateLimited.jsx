...
import { AlertDialog, Card, Flex } from '@covid/core';
import {
  send_transaction,
  rl_set_user_info_action,
} from '../../../modules/message';
import { mojo_to_covid_string, covid_to_mojo } from '../../../util/covid';
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const covidper_input = ip_parsed.limit;
    ...
    const covidper_value = Number.parseInt(Number(covidper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (covid per interval):{' '}
                  {mojo_to_covid_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_covid_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = covid_to_mojo(amount_input.value);
    const fee = covid_to_mojo(fee_input.value);
