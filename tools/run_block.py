...
"""
run_block: Convert an encoded FullBlock from the Covid blockchain into a list of transactions

As input, takes a file containing a [FullBlock](../covid/types/full_block.py) in json format
...
$ca_root is the directory containing your current Covid config files
$hash is the header_hash of the [BlockRecord](../covid/consensus/block_record.py)
...
The information for each spend is an "NPC" (Name, Puzzle, Condition):
        ...
        "conditions": a list of condition expressions, as in [condition_opcodes.py](../covid/types/condition_opcodes.py)
...
from covid.full_node.generator import create_generator_args
...
from covid.wallet.cat_wallet.cat_utils import match_cat_puzzle
