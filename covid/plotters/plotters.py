...
from covid.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from covid.plotters.chiapos import get_chiapos_install_info, plot_covid
from covid.plotters.madmax import get_madmax_install_info, plot_madmax
from covid.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in COVID_ROOT.
    covid_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within COVID_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, covid_plotter, "chiapos", "Covidpos Plotter")
    ...
    if args.plotter == "chiapos":
        plot_covid(args, covid_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, covid_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, covid_root_path, root_path)
