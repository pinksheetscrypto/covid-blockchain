...
from covid.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from covid.wallet.cat_wallet.lineage_store import CATLineageStore
from covid.wallet.lineage_proof import LineageProof
from covid.wallet.payment import Payment
from covid.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            covid_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        covid_tx = dataclasses.replace(covid_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(covid_tx)
    ...
    async def create_tandem_cov_tx(
        self,
        fee: uint64,
        ...
        if fee > amount_to_claim:
            covid_coins = await self.standard_wallet.select_coins(fee)
            origin_id = list(covid_coins)[0].name()
            covid_tx = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
            ...
            assert covid_tx.spend_bundle is not None

            message = None
            for spend in covid_tx.spend_bundle.coin_spends:
            ...
            covid_coins = await self.standard_wallet.select_coins(fee)
            selected_amount = sum([c.amount for c in covid_coins])
            covid_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some COV as part of this
        regular_covid_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_covid_to_claim = payment_amount

        need_covid_transaction = (fee > 0 or regular_covid_to_claim > 0) and (fee - regular_covid_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_covid_transaction:
                    if fee > regular_covid_to_claim:
                        covid_tx, _ = await self.create_tandem_cov_tx(
                            fee, uint64(regular_covid_to_claim), announcement_to_assert=announcement
                        )
                    ...
                    elif regular_covid_to_claim > fee:
                        covid_tx, _ = await self.create_tandem_cov_tx(fee, uint64(regular_covid_to_claim))
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        covid_spend_bundle = SpendBundle([], G2Element())
        if covid_tx is not None and covid_tx.spend_bundle is not None:
            covid_spend_bundle = covid_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, covid_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
        ...
        if covid_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=covid_tx.confirmed_at_height,
                    created_at_time=covid_tx.created_at_time,
                    to_puzzle_hash=covid_tx.to_puzzle_hash,
                    amount=covid_tx.amount,
                    fee_amount=covid_tx.fee_amount,
                    confirmed=covid_tx.confirmed,
                    sent=covid_tx.sent,
                    spend_bundle=None,
                    additions=covid_tx.additions,
                    removals=covid_tx.removals,
                    wallet_id=covid_tx.wallet_id,
                    sent_to=covid_tx.sent_to,
                    trade_id=covid_tx.trade_id,
                    type=covid_tx.type,
                    name=covid_tx.name,
                    memos=[],
                )
