...
from covid.util.chunks import chunks
from covid.protocols import wallet_protocol
from covid.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from covid.protocols.protocol_message_types import ProtocolMessageTypes
from covid.protocols.wallet_protocol import (
...
from covid.server.node_discovery import WalletPeers
from covid.server.outbound_message import Message, NodeType, make_msg
from covid.server.peer_store_resolver import PeerStoreResolver
...
from covid.types.weight_proof import WeightProof, SubEpochData
from covid.util.byte_types import hexstr_to_bytes
from covid.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from covid.util.default_root import STANDALONE_ROOT_PATH
from covid.util.ints import uint32, uint64
from covid.util.keychain import KeyringIsLocked, Keychain
from covid.util.path import mkdir, path_from_root
from covid.wallet.util.new_peak_queue import NewPeakQueue, NewPeakQueueTypes, NewPeakItem
from covid.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from covid.wallet.util.wallet_sync_utils import (
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'covid keys' program.")
            return None
    ...
    async def _process_new_subscriptions(self):
        while not self._shut_down:
            ...
            peer: Optional[WSCovidConnection] = None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.covid.pinksheetscrypto.com"]),
    ...
    def on_disconnect(self, peer: WSCovidConnection):
    ...
    async def is_peer_synced(
        self, peer: WSCovidConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSCovidConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSCovidConnection]:
        if self.server is None:
            return None
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSCovidConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSCovidConnection):
        if self.wallet_state_manager is None:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSCovidConnection) -> int:
        assert self.wallet_state_manager is not None
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSCovidConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSCovidConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
        assert self.wallet_state_manager is not None
        assert self.server is not None
    ...
    async def fetch_puzzle_solution(self, peer: WSCovidConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSCovidConnection] = None
    ) -> List[CoinState]:
        assert self.server is not None
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
    ...
    async def fetch_children(
        self, peer: WSCovidConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
