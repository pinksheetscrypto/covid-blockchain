...
from covid.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
...
from covid.util.check_fork_next_block import check_fork_next_block
from covid.util.errors import Err, ValidationError
from covid.util.ints import uint32, uint128
from covid.util.keychain import Keychain
from covid.util.lru_cache import LRUCache
from covid.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from covid.wallet.settings.settings_objects import BackupInitialized
from covid.wallet.transaction_record import TransactionRecord
from covid.wallet.util.backup_utils import open_backup_file
from covid.wallet.util.wallet_types import WalletType
from covid.wallet.wallet_action import WalletAction
from covid.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSCovidConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSCovidConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSCovidConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSCovidConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSCovidConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSCovidConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
