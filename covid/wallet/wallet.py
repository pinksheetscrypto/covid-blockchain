...
from covid.wallet.puzzles.puzzle_utils import (
...
from covid.wallet.secret_key_store import SecretKeyStore
from covid.wallet.sign_coin_spends import sign_coin_spends
...
from covid.wallet.util.wallet_types import WalletType, AmountWithPuzzlehash
...
class Wallet:
    ...
    async def create_spend_bundle_relative_covid(self, covid_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if covid_amount < 0:
            utxos = await self.select_coins(abs(covid_amount), exclude)
        else:
        ...
        covid_amount = spend_value + covid_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(covid_amount), "memos": []}
                ]
