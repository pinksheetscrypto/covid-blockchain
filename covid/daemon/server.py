...
from covid.cmds.init_funcs import check_keys, covid_init
from covid.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from covid.daemon.keychain_server import KeychainServer, keychain_commands
from covid.daemon.windows_signal import kill
from covid.plotters.plotters import get_available_plotters
from covid.plotting.util import add_plot_directory
from covid.server.server import ssl_context_for_root, ssl_context_for_server
from covid.ssl.create_ssl import get_mozilla_ca_crt
...
from covid.util.service_groups import validate_service
from covid.util.setproctitle import setproctitle
from covid.util.ws_message import WsRpcMessage, create_payload, format_response
...
if getattr(sys, "frozen", False):
    name_map = {
        "covid": "covid",
        "covid_wallet": "start_wallet",
        "covid_full_node": "start_full_node",
        "covid_harvester": "start_harvester",
        "covid_farmer": "start_farmer",
        "covid_introducer": "start_introducer",
        "covid_timelord": "start_timelord",
        "covid_timelord_launcher": "timelord_launcher",
        "covid_full_node_simulator": "start_simulator",
        "covid_seeder": "start_seeder",
        "covid_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if ssl.OPENSSL_VERSION_NUMBER < 0x10101000:
            self.log.warning(
                (
                    ...
                    "A future version of Covid will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
        else:
        ...
        try:
            if Keychain.master_passphrase_is_valid(key, force_reload=True):
                ...
                try:
                    ...
                    self.log.exception("Failed to migrate keys silently. Run `covid keys migrate` manually.")
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["covid", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("covid daemon exiting in 5 seconds")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    if service_command == "covid_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in covid_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    covid_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("covid_daemon")
