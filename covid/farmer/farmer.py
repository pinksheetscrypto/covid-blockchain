...
from covid.pools.pool_config import PoolWalletConfig, load_pool_config, add_auth_key
...
from covid.server.server import ssl_context_for_root
...
from covid.util.config import load_config, save_config, config_path_for_filename, get_config_lock
...
from covid.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'covid keys generate' or open the UI."
        ...
        if "cov_target_address" not in self.config:
            self.config = config["farmer"]
        if "cov_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "cov_target_address" not in self.config or "cov_target_address" not in self.pool_config:
            log.debug("cov_target_address missing in the config")
            return False
