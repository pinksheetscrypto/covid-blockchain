...
from covid.protocols.protocol_state_machine import message_requires_reply
from covid.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
from covid.protocols.shared_protocol import protocol_version
from covid.server.introducer_peers import IntroducerPeers
from covid.server.outbound_message import Message, NodeType
from covid.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from covid.util.errors import Err, ProtocolError
from covid.util.ints import uint16
from covid.util.network import is_in_network, is_localhost
from covid.util.ssl_check import verify_ssl_certs_and_keys
...
class CovidServer:
    def __init__(
        ...
        covid_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        ...
        self.covid_ca_crt_path, self.covid_ca_key_path = covid_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.covid_ca_crt_path, self.covid_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.covid_ca_crt_path, self.covid_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSCovidConnection] = None
        try:
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use covid's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.covidnetwork.org/") as resp:
                    if resp.status == 200:
