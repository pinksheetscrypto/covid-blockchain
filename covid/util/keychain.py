...
from covid.util.keyring_wrapper import KeyringWrapper
...
DEFAULT_USER = f"user-covid-{CURRENT_KEY_VERSION}"  # e.g. user-covid-1.8
DEFAULT_SERVICE = f"covid-{DEFAULT_USER}"  # e.g. covid-user-covid-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting COVID_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("COVID_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from covid.cmds.passphrase_funcs import prompt_for_passphrase
