...
ROOT = pathlib.Path(importlib.import_module("covid").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `covid` file name (which is the executable)
    and the `covid` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `covid` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("covid/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("covid-blockchain"))[0]
...
# TODO: collapse all these entry points into one `covid_exec` entrypoint that accepts the server as a parameter

entry_points = ["covid.cmds.covid"] + [f"covid.server.start_{s}" for s in SERVERS]
...
binaries = [
    (
        f"{ROOT}/madmax/covid_plot",
        "madmax"
    ),
    (
        f"{ROOT}/madmax/covid_plot_k34",
        "madmax"
    )
]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "covid.util.bip39"])
...
if THIS_IS_WINDOWS:
    ...
    binaries = [
        ...
        (
            f"{ROOT}\\madmax\\covid_plot.exe",
            "madmax"
        ),
        (
            f"{ROOT}\\madmax\\covid_plot_k34.exe",
            "madmax"
        ),
        (
...
datas.append((f"{ROOT}/covid/util/english.txt", "covid/util"))
datas.append((f"{ROOT}/covid/util/initial-config.yaml", "covid/util"))
datas.append((f"{ROOT}/covid/wallet/puzzles/*.hex", "covid/wallet/puzzles"))
datas.append((f"{ROOT}/covid/ssl/*", "covid/ssl"))
...
add_binary("covid", f"{ROOT}/covid/cmds/covid.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/covid/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/covid/server/start_{server}.py", COLLECT_ARGS)
