...
from covid.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_covid_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from covid.util.bech32m import encode_puzzle_hash
from covid.util.config import (
    create_default_covid_config,
    initial_config_file,
    load_config,
    save_config,
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'covid keys generate'")
        return None
    ...
    with get_config_lock(new_root, "config.yaml"):
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("cov_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("cov_target_address"):
        ...
        if "cov_target_address" not in config["farmer"]:
            print(
                f"Setting the cov destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
        ...
        if updated_target:
            print(
                f"To change the COV destination addresses, edit the `cov_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    covid_ca_crt, covid_ca_key = get_covid_ca_crt_key()
    covid_ca_crt_path = ca_dir / "covid_ca.crt"
    covid_ca_key_path = ca_dir / "covid_ca.key"
    write_ssl_cert_and_key(covid_ca_crt_path, covid_ca_crt, covid_ca_key_path, covid_ca_key)
    ...
    covid_ca_crt, covid_ca_key = get_covid_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, covid_ca_crt, covid_ca_key, False, overwrite=False)
...
def covid_minor_release_number():
    res = int(covid_version_number()[2])
    print(f"Install release number: {res}")
    return res


def covid_full_version_str() -> str:
    major, minor, patch, dev = covid_version_number()
    ...
    *,
    ...
    if os.environ.get("COVID_ROOT", None) is not None:
        print(
            ...
            f"Please unset the environment variable and run covid init again\n"
    ...
    url = 'https://raw.githubusercontent.com/BTCgreen-Network/covid-blockchain/main/peer_table_node.sqlite'
    mkdir(root_path / "db")
