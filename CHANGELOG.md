...
- `covid farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `covid configure` now supports command line updates to peer count and target peer count.
...
- All Covid repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `covid plotnft` to manage pools.
...
- Correctly display private key in `covid keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `covid keys show`.
...
- Thank you to @CovidMineJP for various improvements.
...
- `covid farm summary` better handles wallet errors.
...
### 1.1.7 Covid Blockchain 2021-06-05
...
## 1.1.6 Covid Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /covid/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `covid configure -t true|false`).
...
## 1.1.5 Covid Blockchain 2021-05-09
...
## 1.1.4 Covid Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `covid keys show`. Use `covid keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Covid Blockchain 2021-05-01
...
## 1.1.2 Covid Blockchain 2021-04-24
...
## 1.1.1 Covid Blockchain 2021-04-21
...
## 1.1.0 Covid Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the Covid Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
...
## 1.0.5 Covid Blockchain 2021-04-14
...
## 1.0.4 Covid Blockchain 2021-04-12
...
- The `src` directory in covid-blockchain has been changed to `covid` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `covid/`
...
## 1.0.3 Covid Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/pinksheetscrypto/covid-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `covid keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chiapos 1.0.0 as well.

## 1.0.2 Covid Blockchain 2021-03-30
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/covid-blockchain).
...
- @jespino changed `covid keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `covid keys show`.
...
## 1.0.1 Covid Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `covid keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/covid-blockchain/).
...
- Users can now pip install e.g. covid-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of Covid Blockchain 2021-03-17

### Added

- This is the first production release of the Covid Blockchain. This can be installed and will wait for the green flag that will be dropped at approximately 7AM PDST (14:00 UTC) on Friday March 19, 2021. All farming rewards from that point forward will be considered valid and valuable COV. There is a six week lock on all transactions. During those six weeks farmers will be earning their farming rewards but those rewards can not be spent.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/covid-blockchain).
...
- Migration with COVID_ROOT set does not crash covid init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TCOV Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.covid.pinksheetscrypto.com/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `covid farm summary` will now display TCOV or COV as appropriate.
...
- `covid farm summary` displayed some incorrect amounts.
...
- `covid netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `covid netspace`. Thanks @altendky.
- There was a typo in `covid plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/pinksheetscrypto/covid-blockchain/issues/1189)
...
- Remove `covid plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `covid keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- PRs and commits to the covid-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TCOV farmed.
- `covid farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `covid wallet` had various bugs.
...
- `covid wallet` now takes get_address to get a new wallet receive address from the CLI.
- `covid plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of covid-blockchain, etc., instead of in blspy.
...
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in covid-blockchain.
...
- Various typos and style clean ups were made to the Click CLI implementation. `covid -upnp f` was added to disable uPnP.
- `covid plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/covid-blockchain).
- You can obtain a new wallet receive address on the command line with `covid wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/pinksheetscrypto/covid-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `covid -h` and `covid SUBCOMMAND -h` can be your guide.
...
- `covid plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `covid netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `covid init` only migrates release candidate directories. The versioned sub directories under `~/covid` will be going away before mainnet.
...
- `covid configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/pinksheetscrypto/covid-blockchain/releases/tag/1.0rc1) was well.
...
- Installers will now be of the pattern CovidSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'covid wallet get_transactions' in the command line to see your transactions.
- 'covid wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Covid Blockchain GUI](https://crowdin.com/project/covid-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/pinksheetscrypto/covid-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/pinksheetscrypto/covid-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `covid plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'covid wallet send' now takes in TCOV or COV as units instead of mojos.
...
- You can now send and receive transactions with the command line. Try `covid wallet -h` to learn more. Also, `covid wallet` now requires a third argument of `show`, therefor you will use `covid wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [covid blockchain gui](https://crowdin.com/project/covid-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `covid init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `covid init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- This release has the final mainnet rewards schedule. During the first three years, each block winner will win 2 TCOV/COV per block for a total of 9216 TCOV per day from 4608 challenges per day.
- Smart transactions now use an announcement instead of 'coin consumed' or lock methods.
- The GUI is now in a separate submodule repository from covid-blockchain, [covid-blockchain-gui](https://github.com/pinksheetscrypto/covid-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `covid-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `covid wallet`, was incorrectly displaying TCOV balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/pinksheetscrypto/covid-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/pinksheetscrypto/covid-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/pinksheetscrypto/covid-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `covid plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- We have removed the '-r' flag from `covid` as it was being overridden in most cases by the `-r` for restart flag to `covid start`. Use `covid --root-path` instead.
- `covid -h` now recommends `covid netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `covid show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `covid configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `covid plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `covid plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `covid show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `covid show` handles sub block 0 better.
...
- `covid plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `covid plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `covid show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `covid configure --enable-upnp`.
...
- `covid configure` now lets you enable or disable uPnP.
...
- `covid netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `covid show` of the GUI use `covid netspace -d 1000` as `covid netspace` defaults to `-d 192` which is one hour.
...
- You can now specify which private key to use for `covid plots create`. After obtaining the fingerprint from `covid keys show`, try `covid plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- There is a new cli command - `covid configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `covid configure -h`. Note that if covid services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the Covid application set to launch on start.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.covid/VERSION/config/`
...
- `covid show -s` has been refactored to support the new consensus.
- `covid netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.covid/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run covid-blockchain.
...
- `covid show -s` now is less brain dead when a node is initially starting to sync.
...
- Partial support for Python 3.9. That includes new versions of Covid dependencies like chiabip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/pinksheetscrypto/covid-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install covid-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/pinksheetscrypto/covid-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/pinksheetscrypto/covid-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The Covid GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/pinksheetscrypto/covid-blockchain/tree/dev/electron-react/src/locales) of the covid-blockchain repository.
- `covid check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- The rpc interfaces of all covid services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test covid since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve covid-blockcovidn's Pypi entry. Thanks to @altendky for this pull request.
...
- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test covid are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.covid` directory.
...
- `covid init` now automatically discovers previous releases in each new release.

### Fixed

- `covid show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `covid show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now Covid Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Covid UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (covid run farmer-only, wallet-only, timelord-only)
...
- `covid show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `covid show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Covid simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Covid implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test covid between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test covid from the earlier two releases however.
...
- `covid show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Covid)
...
- See wallet balances in command line: `covid show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `covid keys sign` and `covid keys verify` to allow farmers to certify their ownership of keys.
...
- `covid start wallet-server` changed to `covid start wallet`, for consistency.
...
- Removed legacy scripts such as covid-stop-server, covid-restart-harvester, etc.
...
- The covid executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./covid -h` from
`~\AppData\Local\Covid-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Covid.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.

### Changed

- Minor changes have been made across the repositories to better support
compiling on OpenBSD. HT @n1000.
- Changed COV units to TCOV units for testnet.
...
"covid plots create" and "covid plots check".
- Add plot directories to config.yaml from the cli with "covid plots add".
...
- The Windows installer is now signed by a Covid Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.covid/test-plots/`
- We have completely refactored all networking code towards making each Covid service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make covid start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Covid Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting covid now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'covid keys' command line.
...
- We added a `covid keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the covid-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `covid netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `covid-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `covid-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `covid-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `covid-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `covid init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `covid start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `covid-start-harvester`.
...
- The user interface now refers to covid coins with their correct currency code of COV.
...
- There was a regression in Covid Proof of Space ([chiapos](https://github.com/pinksheetscrypto/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `covid init` then sync on first start will happen automatically. "\$COVID_ROOT" users will need to delete `$COVID_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `covid netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during covid init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `covid show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `covid show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/pinksheetscrypto/covid-blockchain/wiki) and placed in [INSTALL](https://github.com/pinksheetscrypto/covid-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/pinksheetscrypto/covid-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of covid action commands. You can run `covid version` or `covid start node` for example. Just running `covid` will show you more options. However `covid-create-plots` continues to use the hyphenated form. Also it's now `covid generate keys` as another example.
- Covid start commands like `covid start farmer` and `covid stop node` now keep track of process IDs in a run/ directory in your configuration directory. `covid stop` is unlikely to work on Windows native for now. If `covid start -r node` doesn't work you can force the run/ directory to be reset with `covid start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/pinksheetscrypto/covid-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `covid start wallet-gui` has been chaned to `covid start wallet` which launches but the GUI and server on MacOS and Linux. `covid start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/pinksheetscrypto/covid-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `covid stop all` command sometimes fails, use `covid-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `covid init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `covid-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/pinksheetscrypto/covid-blockchain/wiki/Updating-beta-software). If you've set `$COVID_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `covid show`.

### Changed

- `covid-restart-harvester` has been renamed from `covid-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.covid/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the Covid blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/pinksheetscrypto/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the covid management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `covid-start-farmer &`. The [README.md](https://github.com/pinksheetscrypto/covid-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.covid/release/config, plots/ db/, wallet/ etc. This can be overridden by `export COVID_ROOT=~/.covid` for example which would then put the plots directory in `HomeDirectory/.covid/plots`.
- The libraries covid-pos, covid-fast-vdf, and covid-bip-158 have been moved to their own repositories: [chiapos](https://github.com/pinksheetscrypto/chiapos), [chiavdf](https://github.com/pinksheetscrypto/chiavdf), and [chaibip158](https://github.com/pinksheetscrypto/chiabip158). They are brought in by covid-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/pinksheetscrypto/bls-signatures).
...
- `covid-regenerate-keys` has been renamed `covid-generate-keys`.
...
- Covid fullnode, farmer and harvester now default to logging to covid.log in the covid-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the Covid blockchain.
...
- Added color logging
- Now covid_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to covid_full node, covid_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new covid services were started.
...
[1.0beta4]: https://github.com/pinksheetscrypto/covid-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/pinksheetscrypto/covid-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/pinksheetscrypto/covid-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/pinksheetscrypto/covid-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/pinksheetscrypto/covid-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/pinksheetscrypto/covid-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/pinksheetscrypto/covid-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/pinksheetscrypto/covid-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/pinksheetscrypto/covid-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/pinksheetscrypto/covid-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/pinksheetscrypto/covid-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/pinksheetscrypto/covid-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/pinksheetscrypto/covid-blockchain/releases/tag/Alpha-1.0
